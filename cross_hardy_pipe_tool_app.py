...
def render_solver_results(summary, nodes_df, pipes_df, map_bytes):
    ...
    row1 = st.columns(3)
    row1[0].metric('Worst Node Pressure (psig)', f"{summary['worst_node_pressure_psig']:.3f}")
    row1[1].metric('Worst Node Drop (psi)', f"{summary['worst_node_drop_psi']:.3f}")
    row1[2].metric('Max Segment Velocity (ft/s)', f"{summary['max_segment_velocity_ft_s']:.3f}")

    row2 = st.columns(3)
    row2[0].metric('Max Relative Velocity Metric (scfm/in²)', f"{summary['max_relative_velocity_metric']:.3f}")
    row2[1].metric('Loop Count', str(summary.get('loop_count', '')))
    row2[2].metric('Iterations', str(summary.get('iterations', '')))
    ...
    st.caption(
        ...
        f"Max segment velocity pipe: {summary['max_segment_velocity_pipe_id']} | "
        f"Max relative velocity pipe: {summary['max_relative_velocity_metric_pipe_id']}"
    )
    st.caption('Displayed velocity in ft/s is calculated from the model flow value and inside diameter; the relative velocity metric is retained for comparison and map styling.')
