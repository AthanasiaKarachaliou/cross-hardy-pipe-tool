...
def render_solver_results(summary, nodes_df, pipes_df, map_bytes):
    ...
    c3.metric('Max Relative Velocity Metric (scfm/in²)', f"{summary['max_relative_velocity_metric']:.3f}")
    ...
    st.caption('Relative velocity metric is used for comparison and map styling in the current solver; it is not a true ft/s velocity.')

    display_nodes_df = nodes_df.rename(columns={
        'demand': 'demand_scfm',
    ...
    display_pipes_df = pipes_df.rename(columns={
        'length': 'length_ft',
    ...
    st.dataframe(display_nodes_df, use_container_width=True)

    st.write('### Pipes Results')
    st.dataframe(display_pipes_df, use_container_width=True)

    if map_bytes:
        st.write('### Network Map')
        st.image(map_bytes, caption='Pipe Network Map', use_container_width=True)

    nodes_csv = display_nodes_df.to_csv(index=False).encode('utf-8')
    pipes_csv = display_pipes_df.to_csv(index=False).encode('utf-8')
