...
def render_map_legend():
    st.markdown("""
    <svg width="100%" viewBox="0 0 360 250" xmlns="http://www.w3.org/2000/svg">
      <rect x="0" y="0" width="360" height="250" rx="16" ry="16" fill="#f7f9fc" stroke="#d9e2ef"/>
      <text x="20" y="28" font-size="18" font-weight="700" fill="#1f2d3d">Map Legend</text>

      <text x="20" y="58" font-size="14" font-weight="600" fill="#1f2d3d">Pipe color = relative velocity</text>
      <defs>
        <linearGradient id="velgrad" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stop-color="#2f1e73"/>
          <stop offset="45%" stop-color="#6a00a8"/>
          <stop offset="75%" stop-color="#f1605d"/>
          <stop offset="100%" stop-color="#f6d746"/>
        </linearGradient>
      </defs>
      <rect x="20" y="70" width="230" height="18" rx="9" fill="url(#velgrad)"/>
      <text x="20" y="106" font-size="12" fill="#334e68">Lower relative velocity</text>
      <text x="180" y="106" font-size="12" fill="#334e68">Higher relative velocity</text>

      <text x="20" y="140" font-size="14" font-weight="600" fill="#1f2d3d">Pipe thickness = flow magnitude</text>
      <line x1="20" y1="158" x2="250" y2="158" stroke="#4c78a8" stroke-width="3" stroke-linecap="round"/>
      <line x1="20" y1="186" x2="250" y2="186" stroke="#4c78a8" stroke-width="9" stroke-linecap="round"/>
      <text x="20" y="176" font-size="12" fill="#334e68">Lower flow</text>
      <text x="20" y="206" font-size="12" fill="#334e68">Higher flow</text>

      <text x="20" y="230" font-size="12" fill="#334e68">Arrow direction shows calculated flow direction. Nodes mark connection points.</text>
    </svg>
...
def render_solver_results(summary, nodes_df, pipes_df, map_bytes):
    ...
    pipes_display = pipes_df.rename(columns={
        'length': 'length_ft',
    ...
    c1.metric('Worst Node Pressure (psig)', f"{summary.get('worst_node_pressure_psig', summary.get('worst_node_pressure', 0.0)):.3f}")
    c2.metric('Worst Node Drop (psi)', f"{summary.get('worst_node_drop_psi', summary.get('worst_node_drop', 0.0)):.3f}")
    c3.metric('Max Segment Velocity (ft/s)', f"{summary.get('max_velocity_ft_s', summary.get('max_velocity', 0.0)):.3f}")
    ...
    st.caption(
        f"Reference source: {summary.get('reference_source', '')} @ "
        f"{summary.get('reference_pressure_psig', summary.get('reference_pressure', 0.0)):.3f} psig | "
        f"Worst node: {summary.get('worst_node_pressure_id', '')} | "
        f"Worst drop node: {summary.get('worst_node_drop_id', '')} | "
        f"Max velocity pipe: {summary.get('max_velocity_pipe_id', '')}"
    )
    ...
    if map_bytes:
        ...
        map_col, legend_col = st.columns([4.0, 2.2])
        with map_col:
