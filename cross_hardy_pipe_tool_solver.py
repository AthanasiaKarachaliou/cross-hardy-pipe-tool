...
class Pipe:
    ...
    dP_abs: float = 0.0
    velocity: float = 0.0
    relative_velocity_metric: float = 0.0
    velocity_ft_s: float = 0.0
    loops: str = ''
...
class PipeNetwork:
    ...
    @staticmethod
    def relative_velocity_metric(pipe: Pipe, q: float) -> float:
        area_in2 = math.pi * (pipe.diameter ** 2) / 4.0
        if area_in2 <= 0:
            return 0.0
        return abs(q) / area_in2

    @staticmethod
    def velocity_ft_s(pipe: Pipe, q_scfm: float, avg_pressure_psig: Optional[float],
                      p_std_psia: float = 14.7, t_ratio: float = 1.0) -> float:
        area_ft2 = math.pi * ((pipe.diameter / 12.0) ** 2) / 4.0
        if area_ft2 <= 0:
            return 0.0
        if avg_pressure_psig is None:
            avg_pressure_psig = 0.0
        p_abs_psia = avg_pressure_psig + p_std_psia
        if p_abs_psia <= 0:
            return 0.0
        # Convert standard cfm to actual cfm using ideal-gas pressure correction.
        # Temperature/compressibility are assumed equal to standard unless modeled elsewhere.
        actual_cfm = abs(q_scfm) * (p_std_psia / p_abs_psia) * t_ratio
        actual_cfs = actual_cfm / 60.0
        return actual_cfs / area_ft2
    ...
    def postprocess(self) -> dict:
        for pipe in self.pipes.values():
            ...
            pipe.relative_velocity_metric = self.relative_velocity_metric(pipe, pipe.flow)
            pipe.velocity_ft_s = 0.0
        ...
        for pipe in self.pipes.values():
            p_from = self.nodes[pipe.from_node].pressure if pipe.from_node in self.nodes else None
            p_to = self.nodes[pipe.to_node].pressure if pipe.to_node in self.nodes else None
            valid_pressures = [p for p in [p_from, p_to] if p is not None]
            avg_pressure_psig = sum(valid_pressures) / len(valid_pressures) if valid_pressures else ref.source_pressure
            pipe.velocity_ft_s = self.velocity_ft_s(pipe, pipe.flow, avg_pressure_psig)
        ...
        max_vel_pipe = max(self.pipes.values(), key=lambda p: p.velocity_ft_s)
        max_rel_vel_pipe = max(self.pipes.values(), key=lambda p: p.relative_velocity_metric)
        ...
        return {
            ...
            'max_velocity_ft_s_pipe_id': max_vel_pipe.pipe_id,
            'max_velocity_ft_s': max_vel_pipe.velocity_ft_s,
            'max_relative_velocity_metric_pipe_id': max_rel_vel_pipe.pipe_id,
            'max_relative_velocity_metric': max_rel_vel_pipe.relative_velocity_metric,
        }
    ...
    def pipes_dataframe(self) -> pd.DataFrame:
        ...
        for pipe in self.pipes.values():
            rows.append({
                ...
                'dP_abs': pipe.dP_abs,
                'relative_velocity_metric': pipe.relative_velocity_metric,
                'velocity_ft_s': pipe.velocity_ft_s,
    ...
    def create_map_bytes(self) -> bytes:
        ...
        vel_values = [self.pipes[pid].relative_velocity_metric for pid in self.pipes] or [0.0]
        vmin = min(vel_values)
        vmax = max(vel_values)
        ...
        for pid, pipe in self.pipes.items():
            ...
            t = (pipe.relative_velocity_metric - vmin) / span
            color = plt.cm.plasma(t)
            lw = 1.5 + min(4.0, abs(pipe.flow) / 10.0)
            plt.plot([x1, x2], [y1, y2], color=color, linewidth=lw, zorder=1)
            mx, my = (x1 + x2) / 2.0, (y1 + y2) / 2.0
            label = f"{pid}\nQ={pipe.flow:.2f} scfm\nrel_v={pipe.relative_velocity_metric:.2f} scfm/in²\nv={pipe.velocity_ft_s:.2f} ft/s"
